...
$(document).ready(function() {
    ...
    $('#userList table tbody').on('click', 'td a.linkupdateuser', updateUserInfo);
    
    // Add User button click
    $('#btnAddUser').on('click', maintainUser);    
...
function populateTable() {
    ...
    var sorted = _.sortBy(data, 'username');
    ...
    userListData = sorted;
        ...
        $.each(sorted, function(){
            ...
            tableContent += '<td><a href="/users/' + this.username + '">view</a></td>';
...
function maintainUser(event){
    event.preventDefault();
    var id_ = $('#addUser fieldset input#inputUserId').val(); 
...
function addUser() {
    ...
    var errorCount = 0;
    $('#addUser input:not([type=hidden])').each(function(index, val) {
    ...
    if(errorCount === 0) {
        ...
        $.ajax({
            ...
            if (response.msg === '') {
                ...
                // Tell the server we jave a new user
                socket.emit('refresh users', newUser);
            }
            else {
...
function deleteUser(event) {
    ...
    if (confirmation === true) {
        ...
        $.ajax({
            ...
            // Tell the server to refresh
            socket.emit('refresh users', $(this).attr('rel'));
        });
